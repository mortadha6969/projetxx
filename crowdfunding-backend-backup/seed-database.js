...
const seedDatabase = async () => {
  try {
      ...
      for (let j = 0; j < createdUsers.length; j++) {
        if (createdCampaigns[i].userId !== createdUsers[j].id) {
          ...
          try {
            ...
            await createdCampaigns[i].increment('donors');
            log(`✅ Created donation: $${amount} from ${createdUsers[j].username} to "${createdCampaigns[i].title}"`, colors.green);
          } catch (error) {
            // If the donors column doesn't exist yet, just update the donated amount
            if (error.message.includes('no such column: donors')) {
              log(`⚠️ Warning: 'donors' column not found. Updating only donation amount.`, colors.yellow);
              log(`✅ Created donation: $${amount} from ${createdUsers[j].username} to "${createdCampaigns[i].title}"`, colors.green);
            } else {
              throw error;
            }
    ...
    log('\n=== Database Seeding Complete ===', colors.bright + colors.green);
